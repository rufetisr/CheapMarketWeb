...
const Nutrition = () => {
    ...
    const handleFullAnalysis = async () => {
        if (!image) return toast.info(t('common.loading'));
        console.log(image);
    ...
    const nutriLabels: any = {
        A: t('nutrition.scoreA'),
        B: t('nutrition.scoreB'),
        C: t('nutrition.scoreC'),
        D: t('nutrition.scoreD'),
        E: t('nutrition.scoreE'),
    };

    return (
        <div className="flex flex-col items-center gap-2 mb-4">

            <h1 className="text-3xl font-bold mb-4">{t('nutrition.title')}</h1>
            ...
            <p className="text-gray-700 mb-6 leading-relaxed">
                ...
                <br />
                {t('nutrition.willReceive')}
            </p>

            <ul className="text-gray-700 mb-6 list-disc pl-6 leading-relaxed">
                <li>{t('nutrition.item1')}</li>
                <li>{t('nutrition.item2')}</li>
                <li>{t('nutrition.item3')}</li>
                <li>{t('nutrition.item4')}</li>
                {/* <li>0–100 arası sağlamlıq balı</li> */}
                <li>{t('nutrition.recommendation')}</li>
            </ul>
            ...
            {!loadingOCR && !loadingAnalysis && (
                <>
                    <label className="max-w-lg cursor-pointer w-full p-6 border-2 border-dashed border-gray-400 rounded-xl flex flex-col items-center justify-center hover:bg-gray-100">
                        <p className="text-gray-600 text-lg">📸 {t('nutrition.analyze')}</p>
                        ...
                        {preview && (
                            <div className="mt-4">
                                <p className="text-gray-600 mb-2">{t('nutrition.results')}</p>
                                <img
                                    src={preview}
                                    alt="Preview"
            ...
            {loadingOCR && (
                <div className="mt-4 text-center">
                    <Loader />
                    <p className="text-blue-500 font-medium mt-2">{t('common.loading')}</p>
                </div>
            )}
            ...
            {!loadingOCR && ocrText && (
                <div className="mt-6">
                    ...
                    {/* Editable text */}
                    <h2 className="text-xl font-semibold mb-2">{t('nutrition.results')}</h2>
                    <textarea
                        readOnly
            ...
            {loadingAnalysis && (
                <div className="text-center mt-4">
                    <Loader />
                    <p className="text-blue-500 mt-2">{t('common.loading')}</p>
                </div>
            )}
